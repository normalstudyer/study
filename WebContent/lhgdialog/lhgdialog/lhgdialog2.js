...
(function($, window, undefined) {
	...
	var _setzIndex = function(zIndex) {
		try {
			window["top"].lhgdialog.setting.zIndex = lhgdialog.setting.zIndex;
		} catch (e) {
		}
	};
	...
	lhgdialog.fn = lhgdialog.prototype = {
		...
		/* ! 锁屏 */
		lock : function() {
			var that = this, frm, index = _zIndex() - 1, config = that.config, mask = $(
	...
	/* ! lhgdialog 的全局默认配置 */
	lhgdialog.setting = {
		content : '<div class="ui_loading"><span>加载中...</span></div>',
		title : '\u89C6\u7A97 ', // 标题,默认'视窗'
		button : null, // 自定义按钮
		ok : null, // 确定按钮回调函数
		...
		zIndex : (function(w) {
			try {
				var aa = w['top'].lhgdialog.setting.zIndex;
				return aa;
			} catch (e) {
				return 1976
			}
		})(window), // 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
...
(function($, lhgdialog, undefined) {
	...
	lhgdialog.tips2 = function(content, time, icon, callback) {
		var reIcon = icon ? function() {
			this.DOM.icon[0].innerHTML = '<img src="' + this.config.path
					+ 'skins/icons/' + icon + '" class="ui_icon_bg"/>';
			this.DOM.icon[0].style.display = '';
		...
		var _l = content.length;
		var _cs = _l > 0 ? Math.ceil(_l / .2) : 10;
		return lhgdialog({
			id : 'Tips2',
			zIndex : _zIndex(),
			title : false,
			close : function() {
				var duration = 400, api = this, opt = api.config, wrap = api.DOM.wrap, top = (_$top
						.height()
						/ 5 * 3)
						+ ($(window).scrollTop() - wrap[0].offsetHeight);
				wrap.animate({
							top : top + 'px',
							opacity : 0
						}, duration, function() {
			...
			lock : false,
			resize : false
		}).content(content).position((_$top.width() / 2 - _cs),
				_$top.height() / 5 * 3).time(time || 1.5, reIcon);
	};
